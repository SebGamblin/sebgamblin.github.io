...
function initDeployer() {
  ...
  // ── Point d'entrée : vérifier l'état activé puis démarrer ─────────────────
  ...
  function startWatcher() {
    ...
    if (isOnFileForm()) {
      ...
      const pageObs = new MutationObserver(() => {
        if (isOnFileForm()) { pageObs.disconnect(); watchForZipUpload(); }
      });
      pageObs.observe(document.body, { childList: true, subtree: true });
    }
  }

  // Lire l'état depuis storage avant de démarrer
  ...
  // ── Listener : toggle on/off depuis le popup ───────────────────────────
  browser.runtime.onMessage.addListener((msg) => {
    if (msg.type === 'set-enabled') {
      if (msg.enabled) startWatcher();
      else watcherActive = false; // stoppe les nouvelles surveillances
    }
  });
