...
(function () {
  ...
  function injectDOM() {
    ...
    toolbar.innerHTML =
      ...
      '<button class="moteur-btn" id="moteur-pdf-btn" onclick="window._moteur.printSlidesPdf()" style="display: none">🖨</button>' +
  ...
  function switchMode() {
    ...
    var printSlidesBtn  = document.getElementById('moteur-pdf-btn');
    ...
    if (slideMode) {
      ...
      if (printSlidesBtn) printSlidesBtn.style.display = '';
    } else {
      renderDoc();
      btn.textContent = '▶';
      ...
      if (printSlidesBtn) printSlidesBtn.style.display = 'none';
    }
  }
...
function printSlidesPdf() {
  // Basculer en slides si pas déjà en mode slides
  var wasInDocMode = !slideMode;
  if (wasInDocMode) {
    slideMode = true;
    var btn = document.getElementById('moteur-mode-btn');
    ...
    var printBtn = document.getElementById('moteur-print-btn');
    var infoBtn  = document.getElementById('moteur-info-btn');
    var pdfBtn   = document.getElementById('moteur-pdf-btn');
    if (printBtn) printBtn.style.display = 'none';
    if (infoBtn)  infoBtn.style.display  = 'none';
  ...
  // Attendre que Reveal soit prêt puis imprimer
  setTimeout(function() {
    window.print();
    // Revenir en mode doc après impression
    if (wasInDocMode) {
      setTimeout(function() { switchMode(); }, 500);
    }
  }, 1500);
}
      ...
      (nav.chapters || []).forEach(function(item) {
        if (item.children) {
          ...
          item.children.forEach(function(child) {
            ...
            //var onclick = isHttp ? ' onclick="window._moteur.navigateTo(\'' + child.href + '\');return false;"' : '';
            html += '<li><a href="' + child.href + '"' + active + '>' + child.label + '</a></li>';
          });
          html += '</ul></li>';
        } else {
          var active = item.href === current ? ' class="active"' : '';
          //var onclick = isHttp ? ' onclick="window._moteur.navigateTo(\'' + item.href + '\');return false;"' : '';
  ...
  function navigateTo(href) {
    // Mettre à jour le lien actif dans le sidebar
    document.querySelectorAll('#moteur-sidebar a').forEach(function(a) {
      a.classList.toggle('active', a.getAttribute('href') === href);
    });

    showBanner('<span style="opacity:.5">⏳ Chargement…</span>');

    fetch(href)
      .then(function(r) { return r.text(); })
      .then(function(html) {
        // Extraire le cours-md de la page cible
        var parser = new DOMParser();
        var doc = parser.parseFromString(html, 'text/html');
        var el = doc.getElementById('cours-md');
        if (!el) { hideBanner(); return; }

        // Décoder les entités HTML comme le fait readMarkdownFromPage()
        var txt = document.createElement('textarea');
        txt.innerHTML = el.innerHTML;
        md = txt.value.trim();
      ...
      .catch(function() {
        hideBanner();
        showBanner('<span style="color:red">Erreur de chargement</span>');
      });
  }
  ...
  function init() {
    ...
    document.addEventListener('fullscreenchange', function() {
      ...
      if (document.fullscreenElement) {
        // Entrée en plein écran → cacher sidebar
        if (sidebar) sidebar.classList.add('hidden');
        if (toggle)  toggle.style.left = '12px';
        if (hasSidebar) {
          document.getElementById('moteur-toolbar').style.left = '12px';
          document.getElementById('moteur-doc').style.paddingLeft = '20px';
        }
      } else {
        // Sortie du plein écran → restaurer sidebar
        if (sidebar) sidebar.classList.remove('hidden');
        if (toggle)  toggle.style.left = '252px';
        if (hasSidebar) {
          document.getElementById('moteur-toolbar').style.left = '252px';
          document.getElementById('moteur-doc').style.paddingLeft = '260px';
        }
      }
    });
